...
import { fomratRelativeTime } from "@/utils/format.relative";
    ...
    <div className="flex gap-2 p-6 border-b-2 border-gray-900">
      ...
      <div className="flex-1">
        <div className="flex flex-wrap items-center gap-x-3">
          ...
          <div className="text-xs text-gray-500">@{tweet.user.slug} - {fomratRelativeTime(tweet.dataPost)}</div>
        </div>
        <div className="py-4 text-lg">{tweet.body}</div>
        {tweet.image && (
          <div className="w-full">
            <img src={tweet.image} alt="" className="w-fulll rounded-2xl" />
          </div>
        )}
